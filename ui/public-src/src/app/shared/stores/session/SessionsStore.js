...
export default class SessionsStore {
    ...
    if (this.items) {
      ...
      if (this.nameFilter) {
        filtered = filtered.filter((item) => {
          return (
            item.name.toLowerCase().indexOf(this.nameFilter.toLowerCase()) !==
            -1
          );
        });
      }
  ...
  constructor(appStore, subscriptionManager, queryManager) {
    ...
    this.subscriptionManager.subscribeToAllSessions(this.handleSubscription);
    ...
    const stop = chunkProcessor(
      ...
      (chunk) => {
        ...
        this.logs.replace(this.logs.slice(offset).concat(chunk));
